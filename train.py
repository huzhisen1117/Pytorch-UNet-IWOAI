    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                dists = dists.to(device=device, dtype=torch.float32)
                
                masks_pred = net(images)
                # masks_pred2 = net(images2)
                # masks_pred3 = net(images3)
                # masks_avg = (masks_pred1 + masks_pred2 + masks_pred3) / 3
                

                loss = criterion(masks_pred, true_masks) + dice_loss(masks_pred, true_masks)
                loss2 = dist_loss(masks_pred, true_masks, dists)  * 0.00001
                # loss2 = smooth(masks_pred2, masks_avg) * 500
                ...
                if epoch < dist_epoch:
                    ...
                    pbar.set_postfix(**{'Loss (batch)': loss.item()}) #  'Original loss': loss1.item(), 'Dist loss': loss2.item()})
                
                else:
                
                    l2_w = (epoch - dist_epoch + 1) / (epochs - dist_epoch)
                    loss = loss1 * (1 - l2_w) + loss2 * l2_w
                    ...
                    pbar.set_postfix(**{'Original loss': loss1.item(), 'Dist loss': loss2.item()})
